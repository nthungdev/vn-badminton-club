...
export default function SignUpForm() {
  ...
  return (
    <form className="w-full space-y-3" action={action}>
      ...
      <SubmitButton />
      {state?.signUpError && (
        <p className="text-red-600">{state.signUpError}</p>
      )}
    </form>
  )
}
...
function SubmitButton() {
  ...
  return (
    <button
      type="submit"
      className="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-primary text-white hover:bg-primary-700 focus:outline-none focus:bg-primary-700 focus:ring-primary-400 disabled:opacity-50 disabled:pointer-events-none"
      disabled={pending}
    >
      Sign Up
    </button>
