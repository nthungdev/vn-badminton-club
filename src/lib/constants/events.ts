export const BUTTON_LEAVE_PAST_EVENT_CUTOFF =
  "Because it's close to the event's start time, you cannot leave this event."

export const BUTTON_JOIN_PASSED_JOIN_CUTOFF_TOOLTIP = `Because it's close to the event's start time, you cannot join this event.`
export const BUTTON_LEAVE_PASSED_LEAVE_CUTOFF_TOOLTIP = `Because it's close to the event's start time, you cannot leave this event.`

export const BUTTON_KICK_PASSED_LEAVE_CUTOFF_TOOLTIP = `Because it's close to the event's start time, you cannot kick participants from this event.`
export const BUTTON_ADD_GUEST_PASSED_JOIN_CUTOFF_TOOLTIP = `Because it's close to the event's start time, you cannot add a guest to this event.`

export const BUTTON_CANCEL_EVENT_CONFIRM =
  'Are you sure you want to cancel this event?'
export const BUTTON_LEAVE_EVENT_CONFIRM =
  'Are you sure you want to leave this event?'

export const BUTTON_CONFIRM_CANCEL_EVENT_CONFIRM_HAS_PARTICIPANTS =
  'Are you sure you want to cancel this event? There are participants signed up for this event.'
export const BUTTON_CONFIRM_JOIN_PASSED_EVENT_LEAVE_CUTOFF = `You cannot leave this event after joining it. Are you sure you want to join this event?`
export const BUTTON_CONFIRM_ADD_GUEST_PAST_EVENT_LEAVE_CUTOFF = `You cannot remove the guest after adding them. Are you sure you want to add a guest to this event?`

export const BUTTON_CREATE = 'Create Event'
export const BUTTON_JOIN = 'Join Event'
export const BUTTON_LEAVE = 'Leave Event'
export const BUTTON_EDIT = 'Edit Event'
export const BUTTON_ADD_GUEST = 'Add Guest'
export const BUTTON_CANCEL = 'Cancel Event'
export const BUTTON_KICK = 'Kick Participant'

export const FORM_CREATE_EVENT_TITLE = 'Create Event'
export const FORM_EDIT_EVENT_TITLE = 'Edit Event'

export const KICK_MODAL_EMPTY = 'There are no one else to kick.'

export const EVENT_ADD_GUEST_PROMPT =
  'Enter the name of the guest you would like to add:'
